...
The engine's experimental `ModelContextProtocol`, `Terminal`, and `AllToolsets` plugins are enabled, so a running editor can be driven over MCP.
...
**Gameplay tags are the universal addressing scheme.** Actors are identified to Flow graphs, narrative facts are keyed, and screens/lights are addressed — all by tags registered in `Config/DefaultGameplayTags.ini` (the `Flow.*` namespace: `Flow.Facts.*`, `Flow.Interaction.*`, `Flow.Lights.*`, `Flow.Screens.*`). Register new tags there.
...
- **Gameplay/** — `UInteractionComponent` (world interactable with `Distance` and enabled state; fires `OnUsed`), `USpawnComponent` (spawn point consumed by the spawn Flow nodes).
...
The `Variant_Horror`/`Variant_Shooter` source folders and their Content counterparts are First Person template variants, separate from the main game.
