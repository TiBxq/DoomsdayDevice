...
**Input keys 1–4 are deliberately double-mapped**: `IA_DialogueChoice1-4` (in `IMC_Default`) and `IA_ToolSlot1-4` (in `IMC_Tools`) share keys One–Four, with both contexts added at priority 0. All eight input actions must keep `bConsumeInput = false` — if either set consumes, the other silently never fires. Exclusivity lives in C++ instead: tool input (`OnToolSlotPressed`, and the wheel's `OnToolCycled`, both through `IsToolSwitchBlocked`) early-outs while dialogue choices are on screen (`UBasicUIManager::AreDialogueChoicesPending`, backed by `UDialogueWidget::bChoicesPending` — set in `SetupDialogueChoices`, cleared in `ClearDialogueChoices`), so tools stay switchable during plain dialogue lines and only a live `Choice` node takes the keys; choice selection is a no-op with no pending choices.
...
- **Tools (equippable items on hotkeys 1–4 and the mouse wheel)** — extends the pickup/inventory systems; no separate "tool inventory" exists:
  ...
  - Equip state lives on `ADoomsdayDeviceCharacter`: `ToggleToolSlot(int32)` (same slot toggles off, other slot switches), `UnequipTool()`, `EquipToolSlot(int32)` (exact target; INDEX_NONE = empty hands), `GetCycledToolSlot(FromSlot, Direction)`/`CycleTool(Direction)` (the mouse wheel's ring: `IA_ToolCycle` on `MouseWheelAxis` steps through unlocked slots with empty hands as one stop and wraps — wheel up = previous, wheel down = next), `GetEquippedToolSlot()`/`GetEquippedToolTag()` (INDEX_NONE/empty tag = bare hands). `AToolActor`s (root `USceneComponent` + OnlyOwnerSee first-person static mesh; the mesh's relative transform IS the grip offset) are lazily spawned owner=character, attached to `FirstPersonMesh` socket `HandGrip_R`, and hidden/unhidden on switch — never destroyed until `EndPlay`.
  - Input switches (hotkeys and wheel) are rate-limited by `UPlayerSettings::ToolSwitchDelaySeconds` (default 0.2s) in `ADoomsdayDevicePlayerController::RequestToolSlot`: the first switch is instant; requests during the delay are held — each computed from the held target, not the tool in hands, so notches and repeated presses add up — and applied when the timer ends, skipping the tools in between. Only switches made from input start the delay; pickup auto-equip and the carry auto-unequip bypass it.
...
- **Dialogue arbitration** — `UDialogueSubsystem` (`Public/Dialogue/`) gives the single dialogue screen an owner, so a second dialogue can't overwrite a line or, worse, a pending `Choice`. It is a **`UTickableWorldSubsystem`** on purpose: its state is weak pointers into world-bound Flow node instances, so surviving level travel would carry "busy" into the next level. Nothing here is saved — the nodes restore their own intent.
  - `UFlowNode_BeginDialogue` (pins `Start`/`Stop` → `Started`/`Cancelled`) heads a dialogue and holds the screen for it. Its `BusyPolicy` (`EDialogueBusyPolicy`) is **Wait** (default, FIFO queue), **Interrupt** (cancel whoever holds it) or **Drop** (never play). The node stays active for the whole dialogue — it *is* the session handle — and `Cleanup` → `ReleaseIfOwner` means any teardown path frees the screen.
  - **Always wire `Cancelled`.** It is the only exit for a dropped or interrupted dialogue; inside a sub-graph it must reach `Finish` or the parent's `UFlowNode_SubGraph` stays active forever. `ValidateNode` errors if it dangles.
  - `_DialogueLine`/`_Choice` call `TryJoinSession` in `ExecuteInput` and `LeaveSession` in `Cleanup`. Membership is by **Flow asset instance lineage** (walks `UFlowAsset::GetParentInstance()`), so a `BeginDialogue` in a parent graph owns lines in its sub-graphs. A refused node calls `Finish()` and displays nothing — that is what stops an orphaned branch painting over the live dialogue.
  - **Legacy dialogues with no `BeginDialogue` still work**: the first line opens an *implicit* session (logged once as a Warning), which can be waited on and interrupted like any other. A second untracked dialogue overlapping it is refused and its branch stops — the migration incentive.
  - `_EndDialogue` no longer closes the screen itself; it calls `RequestEndDialogue`. An `EndDialogue` that does **not** own the screen (typically one on a cancelled dialogue's `Cancelled` branch) passes straight through without touching the UI — load-bearing, or it would close the interrupter's screen.
  - Two ordering rules the design depends on. **The screen is released on close-*finished*, never when `EndDialogue` starts** — `CloseDialogue()` is an async Blueprint animation round-trip, so releasing early lets the next dialogue open a widget the old one then destroys. And **`Cancelled` is fired from `Tick`, never inline**: `bDeferTriggeredOutputsWhileTriggering` defaults to true and its scope stack is per asset instance, so triggering a pin on a node in another instance would run that whole branch immediately, making execution order depend on graph topology.
  - Safety nets: a `bArbitrating` guard downgrades an interrupt raised from inside an interrupt, and a watchdog (`UPlayerSettings::DialogueCloseTimeoutSeconds`, default 2s) force-closes via `UBasicUIManager::ForceCloseDialogueWidget()` if a widget Blueprint never calls `FinishDialogueClose`. Console: `DumpDialogueState`; log category `LogDialogueSession`.
