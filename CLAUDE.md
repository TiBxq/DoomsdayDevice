...
- **Tools (equippable items on hotkeys 1–4 and the mouse wheel)** — extends the pickup/inventory systems; no separate "tool inventory" exists:
  ...
  - Equip state lives on `ADoomsdayDeviceCharacter`: `ToggleToolSlot(int32)` (same slot toggles off, other slot switches), `UnequipTool()`, `CycleTool(int32 Direction)` (mouse wheel via `IA_ToolCycle` on `MouseWheelAxis`; steps through unlocked slots with empty hands as one stop and wraps — wheel up = previous, wheel down = next), `GetEquippedToolSlot()`/`GetEquippedToolTag()` (INDEX_NONE/empty tag = bare hands). `AToolActor`s (root `USceneComponent` + OnlyOwnerSee first-person static mesh; the mesh's relative transform IS the grip offset) are lazily spawned owner=character, attached to `FirstPersonMesh` socket `HandGrip_R`, and hidden/unhidden on switch — never destroyed until `EndPlay`.
